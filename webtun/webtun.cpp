...
const char default_url[] = "http://tdlive.ignorelist.com/vlt/vltdir.php" ;
...
void loadUrl()
{
	...
	if( strncmp( (char *)v, "http://", 7 )==0 ) {
		...
		// other dvr would store live server as: [network] : td_url=142.112.102.220       
		v = dvrconfig.getvalue("network", "td_url") ;
		if( v.length() > 6) {
			string b ;
			sprintf(b.setbufsize(1000),"http://%s/tdc/livetun.php", (char *)v);
			g_webtun_url = (char *)b ;
		}
		else {
