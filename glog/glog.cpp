	...
	int find()
	{
		...
		if (m_pdir) {
			while (readdir_r(m_pdir, &m_entry, &m_pent) == 0) {
				...
				if ((m_pent->d_name[0] == '.' && m_pent->d_name[1] == '\0') || (m_pent->d_name[0] == '.' && m_pent->d_name[1] == '.' && m_pent->d_name[2] == '\0')) {
					continue;
				}
...
int sensor_log()
{
	...
	if (gforce_log_enable) {
		if (p_dio_mmap->gforce_log0) {
			if (fabs(p_dio_mmap->gforce_forward_0) > 0.3 || fabs(p_dio_mmap->gforce_right_0) > 0.3 || fabs(p_dio_mmap->gforce_down_0 - 1.0) > 1.0) {
		...
		if (p_dio_mmap->gforce_log1) {
			if (fabs(p_dio_mmap->gforce_forward_1) > 0.3 || fabs(p_dio_mmap->gforce_right_1) > 0.3 || fabs(p_dio_mmap->gforce_down_1 - 1.0) > 1.0) {
				// we record forward/backward, right/left acceleration value, up/down as g-force value
				gps_logprintf(16, ",%.1f,%.1f,%.1f",
