...
void check_rtccmd()
{
    ...
    if (p_dio_mmap->rtc_cmd != 0)
    {
        ...
        else if (p_dio_mmap->rtc_cmd == 10)
        { // Ex command, calibrate gforce sensor
#ifdef SUPPORT_YAGF
            yagf_calibration();
#else
            gforce_calibration();
        ...
        else if (p_dio_mmap->rtc_cmd == 11)
        { // Ex command, save gforce crash data
        ...
        else if (p_dio_mmap->rtc_cmd == 15)
        { // Ex command, calibrate gforce mount angle
            p_dio_mmap->rtc_cmd = 0;
            gforce_calibrate_mountangle(1);
        }
        else if (p_dio_mmap->rtc_cmd == 16)
        { // Ex command, stop calibrate gforce mount angle
            p_dio_mmap->rtc_cmd = 0;
...
void check_cpuusage()
{
    ...
    if ((runtime - cpuusage_timer) > 5000)
    { // 5 seconds to monitor cpu usage
...
void check_watchdog()
{
    ...
    if (usewatchdog && (runtime - watchdog_timer) > 3000)
    { // 3 seconds to kick watchdog
