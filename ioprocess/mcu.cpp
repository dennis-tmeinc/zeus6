...
//     return 0: no data
//            1: yes
int mcu_dataready(int usdelay)
{
    if (mcu_handle <= 0) {
        return 0;
    }
    return serial_dataready(mcu_handle, usdelay);
}
...
void mcu_clear(int usdelay)
{
    int i;
    char buf[100];
    for (i = 0; i < 1000000; i++) {
        if (mcu_read(buf, sizeof(buf), usdelay, usdelay) == 0) {
            break;
        }
    }
}
...
char mcu_checksum(char* data)
{
    ...
    if (datalen < 6 || datalen > 120) {
        return (char)-1;
    }
    unsigned char ck = 0;
    while (datalen-- > 0) {
        ck += (unsigned char)*data++;
    }
    return (char)ck;
}
...
static char* mcu_recvmsg(int rdelay)
{
    ...
    int err = 0;
    while (mcu_dataready(rdelay)) {
        if (mcu_read1(recvbuf + recvsiz)) {
            recvsiz++;
            if (recvsiz > 5) {
...
// return : received msg
//          NULL : failed
static char* mcu_recv(int usdelay)
{
    char* rmsg;
    while ((rmsg = mcu_recvmsg(usdelay)) != NULL) {
...
static char* mcu_waitrsp(int target, int cmd)
{
    int retry;
    for (retry = 0; retry < 10; retry++) {
        char* rsp = mcu_recv(500000); // half second wait for response
        if (rsp) {
