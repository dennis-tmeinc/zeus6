	...
	if (m_attr.GPS_enable)
	{
		...
		if (m_attr.showgpslocation)
		{
			...
			if (gps_location(&lati, &longi))
			{
				...
				sprintf(osdbuf, "%d\xb0%02d\'%02d\"%c %d\xb0%02d\'%02d\"%c ",
...
void cap_init()
{
	...
	for (i = 0; i < cap_channels; i++)
	{
		...
		if (cameratype == CAP_TYPE_HIKIP)
		{ // eagle ip camera
			cap_channel[i] = new ipeagle32_capture(i);
		}
#ifdef SUPPORT_IPCAM
		else if (cameratype == CAP_TYPE_HDIPCAM)
		{ // standard HD ip camera
			cap_channel[i] = new Ipstream_capture(i);
		}
#endif
		else
		{ // by default: CAP_TYPE_HIKLOCAL
