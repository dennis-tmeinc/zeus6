...
int dvrfile::open(const char *mode)
{
	...
	else if (*mode == 'r')
	{
		...
		if (m_fileheader.flag == ((struct hd_file *)&Dvr264Header)->flag)
		{
...
int dvrfile::repairpartiallock()
{
	...
	if (locklength >= filelen || locklength < 2)
	{ // not a partial locked file
		return 0;
	}
...
int dvrfile::breakLfile(char *filename, struct dvrtime *locktime)
{
	...
	{ // to lock whole file
		i = strlen(lockdisk);
		if (strncmp(lockdisk, filename, i) == 0)
		{ // same disk, rename it
...
void file_init()
{
	...
	file_bufsize &= ~(4095);
	if (file_bufsize < 256 * 1024)
		file_bufsize = (256 * 1024);
	if (file_bufsize > (4 * 1024 * 1024))
